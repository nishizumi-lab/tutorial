...
- *風力発電*
    ...
    - 風車の受風面積をA[m2]、風速をv[m／s] とすると、単位時間当たりの出力P[W]は以下のようになる。
        - $ P=\frac{1}{2}\ro A v^3 $
    - 出力Pの導出について、単位時間当たりに通過する風の体積V [m3／s] は$V=Av [m^3/s]$となるので、単位時間当たりに通過する風の質量m[kg/m3] は，空気の密度をρ[kg/m3] とすると，$m = \rho V = \rho Av$となる。物体の運動エネルギーE(=風車の出力)は$E=\frac{1}{2}mv^2=\frac{1}{2} \rho A v^3$となる。
- *洋上風力発電*
